...
如： `ssh-keygen -t rsa -C "feaplat" -f id_rsa` 然后一路回车，不要输密码 最终生成 `id_rsa`、`id_rsa.pub` 文件，复制`id_rsa.pub`文件内容到git仓库，复制`id_rsa`文件内容到feaplat爬虫管理系统
...
自己随便搞事情，搞完修改下 `.env`文件里的 SPIDER\_IMAGE 的值即可
