...
> 备注：建议以 key/value（object { : string | \[string\] }）配置 entry。详情参考 Webpack 文档：关于 entry 的配置方法
...
> 详情参考 Webpack 文档：关于 resolve.extensions 的配置方法
...
> 详情参考 Webpack 文档：关于 resolve.alias 的配置方法
...
### 6) 依赖打包策略（忽略 node\_modules）

> 打包过程中忽略 node\_modules 依赖，减少最终体积；可通过 allowList 指定需注入 bundle 的依赖。

module.exports = {
  ...
  webpack: {
    ignoreNodeModules: true, // 是否忽略node\_modules中的依赖文件
...
> 关于 params-replace-loader 的使用方法
