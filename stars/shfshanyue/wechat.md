...
project: wechat
stars: 23
description: 黑客增长: 从博客引流到公众号
url: https://github.com/shfshanyue/wechat
---
...
关于部署，则使用 `docker`，`docker-compose` 以及 `traefik`。至于部署这块，我有一个基础设施很完善的服务器环境，可以参考文章 当我有一台服务器时我做了什么。
...
此时使用 `JWT` 来做用户认证。因此校验口令时返回生成的 `jwt`，在浏览器端持久化，使用它来保持用户状态。关于 `JWT`，可以看我以前的文章 JWT 深入浅出
...
而本篇文章，属于 个人服务器运维指南 的案例篇，关于 `docker`，`compose` 及 `traefik` 等基础设施的搭建均在本系列中有所介绍
