...
4.使用chmod 777 biuld.sh加权限安装所需依赖,根据系统环境安装,如果是centos安装build\_centos7.sh得,如果是ubutu,就按照build\_ubutu.sh
...
6.新增自动化定时检测是否卡死的功能代码,请在执行上述自动化指令后手动执行该代码(运行该命令之前,请先运行build.sh文件或者手动在shell执行命令:sed -i "s/\\r//" check\_nohup\_size.sh,定时检查nohup.out是否变化防止卡死导致自动化停止)
