---
project: tencent-nestjs
stars: 25
description: null
url: https://github.com/serverless-components/tencent-nestjs
---
...
// 注意: 通过注入 NODE\_ENV 为 local，来方便本地启动服务，进行开发调试
const isLocal \= process.env.NODE\_ENV \=== 'local'
if (isLocal) {
  bootstrap().then((app) \=> {
    app.listen(3000, () \=> {
...
> **注意：**
> 
> 1、如您的账号未 登录 或 注册 腾讯云，您可以直接通过 `微信` 扫描命令行中的二维码进行授权登陆和注册。
> 
...
如果已有腾讯云账号，可以在API 密钥管理中获取 `SecretId` 和`SecretKey`.
