...
\# Load the model
model\_id \= "./HunyuanImage-3"
\# Currently we can not load the model using HF model\_id \`tencent/HunyuanImage-3.0\` directly 
\# due to the dot in the name.

kwargs \= dict(
    attn\_implementation\="sdpa",     \# Use "flash\_attention\_2" if FlashAttention is installed
    trust\_remote\_code\=True,
    torch\_dtype\="auto",
    device\_map\="auto",
    moe\_impl\="eager",   \# Use "flashinfer" if FlashInfer is installed
)

model \= AutoModelForCausalLM.from\_pretrained(model\_id, \*\*kwargs)
model.load\_tokenizer(model\_id)

\# generate the image
prompt \= "A brown and white dog is running on the grass"
image \= model.generate\_image(prompt\=prompt, stream\=True)
...
# set env
export DEEPSEEK\_KEY\_ID="your\_deepseek\_key\_id"
export DEEPSEEK\_KEY\_SECRET="your\_deepseek\_key\_secret"

python3 run\_image\_gen.py --model-id ./HunyuanImage-3 --verbose 1 --sys-deepseek-prompt "universal" --prompt "A brown and white dog is running on the grass"
...
# Set your model path
export MODEL\_ID="path/to/your/model"
...
sh run\_app.sh --moe-impl flashinfer --attn-impl flash\_attention\_2
...
-   **system\_prompt\_universal**: This system prompt converts photographic style, artistic prompts into a detailed one.
-   **system\_prompt\_text\_rendering**: This system prompt converts UI/Poster/Text Rending prompts to a deailed on that suits the model.
