...
When set to `true`, the calendar is auto-closed when picking a day \_(or a time if `time: true` and `date: false`). A value of `'time'` will only auto-close the calendar when a time is picked.
...
Format used to display weekdays. Takes `min` _(Mo)_, `short` _(Mon)_, `long` _(Monday)_, or an array with seven strings of your choosing.
...
The calendar has been `.restore`d

`destroyed`

`[]`

The calendar has been `.destroy`ed
...
The calendar has been displayed

`hide`

`[]`

The calendar has been hidden

`back`

`[month]`

The calendar view has been moved back a month to the value `moment.month()`

`next`

`[month]`

The calendar view has been moved forward a month to the value `moment.month()`
