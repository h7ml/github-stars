...
3.  **配置环境变量** 在项目根目录下创建 `.env.local` 文件，并根据你的实际环境配置以下变量：
    
    # UniAPI 的核心配置文件路径 (绝对路径)
    API\_YAML\_PATH\=/path/to/your/uniapi/config/api.yaml
    
    # --- 数据库配置 (二选一) ---
    
    # 1. 使用 SQLite (默认)
    STATS\_DB\_TYPE\=sqlite
    # UniAPI 生成的统计数据库文件路径 (绝对路径)
    STATS\_DB\_PATH\=/path/to/your/uniapi/data/stats.db
    
    # 2. 使用 PostgreSQL
    # STATS\_DB\_TYPE=postgres
    # STATS\_DB\_HOST=localhost
    # STATS\_DB\_PORT=5432
    # STATS\_DB\_USER=your\_postgres\_user
