...
参考 docker-compose.yml 和 docker-compose.sqlite.yml
...
**必填** 数据库地址，例如 `mysql://root:123456@127.0.0.1:3306/wewe-rss`

\-

`DATABASE_TYPE`

数据库类型，使用 SQLite 时需填写 `sqlite`

\-

`AUTH_CODE`

服务端接口请求授权码，空字符或不设置将不启用 (`/feeds`路径不需要)
...
输出模式，可选值 `fulltext` (会使接口响应变慢，占用更多内存)
...
账号被封控，等一天恢复。账号正常时可通过重启服务/容器清除小黑屋记录
